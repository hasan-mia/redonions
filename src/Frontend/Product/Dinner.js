...
import './Product.css'
const Dinner = ({dinner}) => {
	const {title, img, description, price, id} = dinner
	const dinnerDetails = useNavigate()
	return (
		<div className="product-card flex flex-col items-center rounded-lg p-4">
			...
			<div className="flex justify-between w-full">
				...
				<button onClick={()=>dinnerDetails(`/dinners/${id}`)} className='btn-buy px-2 py-1 text-xl border-2 rounded-3xl font-semibold'>Buy Now</button>
			</div>
		</div>
	);
};
