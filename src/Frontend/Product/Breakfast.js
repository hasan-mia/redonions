...
const Breakfast = ({breakfast}) => {
	...
	const breakfastDetails = useNavigate()
	return (
		<div className="product-card flex flex-col items-center rounded-lg p-4">
			...
			<div className="flex justify-between w-full">
				...
				<button onClick={()=>breakfastDetails(`/breakfast/${id}`)} className='btn-buy px-2 py-1 text-xl border-2 rounded-3xl font-semibold'>Buy Now</button>
			</div>
		</div>
	);
};
